          ...
          >
            <template v-slot:item="{ item }">
              <tr>
                <td>{{ item.name }}</td>
                <td>
                  <v-card class="elevation-5 my-2 mx-auto" width="50" height="50"  :color="item.code"></v-card>
                  <p class="my-auto">{{ item.code }}</p>
                </td>
                ...
                <td>
                  <div class="d-flex justify-end align-center">
                    <v-btn
                      ...
                      elevation="3"
                      class="mr-3"
                      @click="(dialogEdit = true), (currentColor = item)"
                      x-small
                      color="blue-grey"
                    >
                    ...
                    <v-btn
                      ...
                      dark
                      elevation="3"
                      @click="(dialogDelete = true), (colorId = item.id)"
                      x-small
                      color="red"
                    >
      ...
      <v-card>
       <v-card-title primary-title style="background-color:#85c9ff; font-size: 18px">
        ...
        <div class="d-flex justify-end py-3 pr-5">
          <v-btn color="primary" @click="deleteColor" text>Đồng ý</v-btn>
...
export default {
  ...
  data() {
    return {
      headers: [
        ...
        { text: "Màu", value: "code", align: "center" },
  ...
  mounted() {
    this.$store.dispatch("Color/GetAllColor");
  },
  computed: {
    ...mapGetters("Color", ["listColor"]),
  },
  methods: {
    convertDate(date) {
